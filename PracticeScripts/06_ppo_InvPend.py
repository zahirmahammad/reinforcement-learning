...
class PPO_BiPedWalker:
    def __init__(self):
        self.eval_env = gym.make("InvertedPendulum-v5", render_mode='rgb_array')
    ...
    def TrainAgent(self):
        ...
        obs = torch.from_numpy(self.vec_env.reset()[0]).float().to(DEVICE)
        ...
        for ep in range(epochs):
            for step in range(NUM_STEPS):
                ...
                for idx, d in enumerate(done):
                    if d:
                        obs[idx] = torch.from_numpy(self.vec_env.envs[idx].reset()[0]).float().to(DEVICE)
            ...
            with torch.no_grad():
                ...
                for r in reversed(range(NUM_STEPS)):
                    if r == NUM_STEPS - 1:
                        ...
                        next_value_state = self.values_arr[r + 1]
